...
Last updated: 2026-05-19 (Strict Concurrency enabled and Dashboard Overlay extraction complete)
...
- `Strict Concurrency & Dashboard view declutter`: Set `SWIFT_STRICT_CONCURRENCY` to `complete` and `SWIFT_TREAT_WARNINGS_AS_ERRORS` to `YES` in the target configurations, verifying a completely clean compile-time profile. Extracted 4 major computed overlays (`ScreenLockOverlay`, `RideTipsOnboardingOverlay`, `IndoorEndWorkoutOverlay`, `PersistenceErrorOverlay`) from `DashboardView.swift` into `DashboardOverlayViews.swift`.
